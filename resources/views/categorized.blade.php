...
@section('content')
                ...
                @if (request()->has('keyword') && $category->title)
                    <div class="d-flex justify-content-between">
                        <h5 class=" mb-2">Showing results for <span class="fw-bold">{{request()->keyword}}</span> and '<span class="fw-bold">{{$category->title}}</span>' Category</h5>
                        <a href="{{route('index')}}" class="text-dark"><span>cancel</span></a>
                    </div>
                @elseif ($category->title)
                    <div class="d-flex justify-content-between">
                        <h5 class=" mb-2">Showing results for '<span class="fw-bold">{{$category->title}}</span>' Category</h5>
                        <a href="{{route('index')}}" class="text-dark"><span>cancel</span></a>
                    </div>
                @endif
